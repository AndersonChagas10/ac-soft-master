...
export class UsuariosService {
  ...
  usuarios(): Observable<User[]> {
    return this.http.get<User[]>(`${AC_API}/${VERSION_API}/usuario`);
  }

  buscarFinalizadores(): Observable<User[]> {
    return this.http.get<User[]>(`${AC_API}/${VERSION_API}/finalizadores`);
  }

  perfisUsuario(): Observable<PerfilUsuario[]> {
    return this.http.get<PerfilUsuario[]>(`${AC_API}/${VERSION_API}/perfil`);
  }

  usuarioById(id: string): Observable<User> {
    return this.http.get<User>(`${AC_API}/${VERSION_API}/usuario/${id}`);
  }

  salvarUsuario(user: User): Observable<any> {
    return this.http.post(`${AC_API}/${VERSION_API}/usuario`,
                          JSON.stringify(user))
